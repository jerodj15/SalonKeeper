...
public class AdminReportsController{
    ...
    @FXML
    private ComboBox stylistRetentionCB;
    @FXML
    ...
    @FXML
    private Button retentionCalculateButton;
    @FXML
    ...
    @FXML
    private Button typeCalculateButton;
    @FXML
    private Button typeClearButton;
    @FXML
    private ComboBox speedStylistCB;
    @FXML
    private DatePicker speedStartDP;
    @FXML
    private DatePicker speedEndDP;
    @FXML
    private Button speedCalculateButton;
    @FXML
    private Button speedClearButton;
    @FXML
    private ComboBox speedSubTypeCB;
    @FXML
    private ComboBox speedTypeCB;
    @FXML
    ...
    List<List> allStylists = DBRequest.getActiveEmployees();
    ...
    // // Method for initial setup of the administrator reports screen
    public void setupAdminReports(SalonKeeper mySalonKeeper)
    {
        System.out.println(allStylists);
    ...
    public void setupClientRetention()
    {
        ...
        int stylistId;
        stylistsInformation = new ArrayList<>();
        System.out.println(allStylists);
        // Get first names
        ...
        stylistRetentionCB.getItems().clear();
        stylistRetentionCB.getItems().setAll(stylistsInformation);
        
    }
    // Method for calculating retention ------TEST ME ---------------
    public void calculateRetentionPressed()
    {
        ...
        List<Object> eInfo = (List<Object>) stylistRetentionCB.getSelectionModel().getSelectedItem();
        employeeID = (int) eInfo.get(0);
    ...
    public void clearRetentionButtonPressed()
    {
        ...
        stylistRetentionCB.getSelectionModel().clearSelection();
    ...
    public void setupServicesPerformed()
    {
        ...
        typeStylistCB.getItems().clear();
        typeStylistCB.getItems().setAll(stylistsInformation);
    }
    // Method for caculating the service amount ------------TEST ME-------------
    public void calculateServiceAmountPressed()
    {
        ...
        // Add items to list to search
        List<Object> searchStuff = new ArrayList<>();
        List<Object> eInfo = (List<Object>) typeStylistCB.getSelectionModel().getSelectedItem();
        employeeID = (int) eInfo.get(0);
        searchStuff.add(employeeID);
        searchStuff.add(startingDate);
        searchStuff.add(endingDate);
    ...
    // Method for clearing the search criteria on the service type tab
...
// Setup the appointment history tab *****************************************************************************
    public void setupApptHistory()
    {
        ...
        apptHistoryCB.getItems().clear();
        apptHistoryCB.getItems().setAll(stylistsInformation);
    }
    // Method for clearing all the search criteria
    public void clearAptHistPressed()
    {
        apptStartDP.getEditor().clear();
        apptEndDP.getEditor().clear();
        apptTableView.getItems().clear();
        apptHistoryCB.getSelectionModel().clearSelection();
    }
    // Method for retrieving the appointment history and populating the tableview ----TEST ME------
    ...
    TableColumn<Appointment, String> dateColumn = new TableColumn("Date of Service");
        ...
        List<Object> selectedStylistObjects = (List<Object>) apptHistoryCB.getSelectionModel().getSelectedItem();
        
        int stylistID = (int) selectedStylistObjects.get(0);
